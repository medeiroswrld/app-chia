    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-brand-dark tracking-tight">
          Bem-vinda, <span className="text-brand-green">{user?.email?.split('@')[0] || 'Linda'}</span>!
        </h2>
        <p className="text-gray-500 text-sm mt-1">Pronta para continuar seu protocolo hoje?</p>
      </div>

      {/* Main Action Card */}
      <div className="bg-gradient-to-br from-brand-green to-[#388E3C] rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-green-50 text-sm mb-6 max-w-[80%]">
            Marque suas tarefas de hoje e acompanhe sua evolução diária.
          </p>
          <button 
            onClick={() => navigate('/protocolo')}
            className="bg-white text-brand-green px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-green-50 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="card text-center py-5">
          <div className="text-3xl font-bold text-brand-dark mb-1">{completedDays}</div>
          <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">Dias Concluídos</div>
        </div>
        <div className="card text-center py-5">
          <div className="text-3xl font-bold text-brand-dark mb-1">{recipes.length}</div>
          <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">Receitas Inclusas</div>
        </div>
      </div>

      {/* Quick Links */}
      <div>
        <h3 className="font-bold text-brand-dark mb-3">Acesso Rápido</h3>
      ...
      {/* Checklist Oferta */}
      <div className="card">
        <h3 className="font-bold text-brand-dark mb-4">Sua Oferta Inclui</h3>
        <ul className="space-y-3">
          {[
            'Receitas com CHIA para secar',
      ...
      <div 
        ...
        className="card border-2 border-slate-100 cursor-pointer hover:border-gray-200 transition-colors flex items-center justify-between"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-brand-dark text-sm">Grupo VIP</h4>
            <p className="text-xs text-brand-orange mt-0.5">Em desenvolvimento</p>
          </div>
        </div>
